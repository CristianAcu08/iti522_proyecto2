...
// Configuración
const API_BASE_URL = '/api'; // En desarrollo local, asumimos mismo origen. En producción, usar URL completa de la API
// Si se despliega en distinto dominio, cambiar a: const API_BASE_URL = 'https://tuapp.azurewebsites.net/api';
...
function showDeleteConfirmation(id) {
    const product = products.find(p => p.id === id);
    if (product) {
        deleteMessage.textContent = `¿Está seguro de que desea eliminar "${product.name}"?`;
...
function showFormMessage(message, type) {
    ...
    productForm.insertBefore(msgDiv, saveBtn);
